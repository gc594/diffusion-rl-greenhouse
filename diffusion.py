...
class Diffusion(nn.Module):
    def __init__(self, state_dim, action_dim, model, max_action,
                 beta_schedule='linear', n_timesteps=50,
    ...
    def sample(self, state, *args, **kwargs):
        ...
        if t_step_res:
            # return F.softmax(action, dim=-1), F.softmax(t_step_res, dim=-1)  # Revise
            return action.tanh(), t_step_res.tanh()  # Revise
        else:
            # return F.softmax(action, dim=-1)  # Revise
            return action.tanh()
